  ...
  this.http.get(url_containers,  { headers })
    .pipe(map(r => { console.log(r);
      ...
      JSON.parse(data).containers.forEach(element => {
        i++;
      var url_risk = url + "/api/graphql?query={containers(where:{path:%22RootId%22,comparison:%22equal%22,value:%22" + element.id + "%22}){name,assets{id,name,group,evidences{id,name},vulnerabilities{id,name},risks{id,name,payload{stride,lindun, impact, likelihood},vulnerabilities{id,name}, treatments{id,payload,definition{id,type,description,createdDateTime}}},treatments{id,payload}},edges{id,fromId,toId}}}";
